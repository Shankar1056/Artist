...
public class RequestsActivity extends AppCompatActivity {
                ...
                if(!response.equals("")) {
                    ...
                    try {
                        for (int j = 0; j < new JSONObject(response).getJSONObject(JsonParser.DATA).getJSONArray(JsonParser.PENDING).length(); j++) {
                            RequestsModel model=new RequestsModel(new JSONObject(response).getJSONObject(JsonParser.DATA).getJSONArray(JsonParser.PENDING).getJSONObject(j));
                            model.setCustomer_id(new JSONObject(response).getJSONObject(JsonParser.DATA).getJSONArray(JsonParser.PENDING).getJSONObject(j).getString(JsonParser.CUSTOMER_ID));
                            models.add(model);
                        }
    ...
    {
        adapter=new RequestsAdapter(models);
